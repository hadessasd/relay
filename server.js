...
function geoLookup(ip) {
    return new Promise(resolve => {
        ...
        const req = http.get(`http://ip-api.com/json/${cleanIp}?fields=country,city,regionName`, res => {
            let raw = '';
            res.on('data', c => raw += c);
            res.on('end', () => {
                try {
                    const j = JSON.parse(raw);
                    resolve({ country: j.country || '?', city: j.city || '?', region: j.regionName || '' });
...
/* POST /whitelist/verify
   Body: { machineId: "raw_serial", computerName: "hostname" }
...
app.post('/whitelist/verify', async (req, res) => {
    ...
    if (found) {
        ...
        if (found.status === 'pending') {
            return res.json({ authorized: false, pending: true });
        }
        /* denied */
        return res.json({ authorized: false, pending: false, denied: true });
    }

    /* Brand new machine — not seen before */
    return res.json({ authorized: false, pending: false });
});

/* POST /whitelist/request
   Body: { machineId: "raw_serial", computerName: "hostname", reason: "..." }
...
app.post('/whitelist/request', async (req, res) => {
    ...
    console.log(`[whitelist] new pending: ${computerName} (${ip}) ${geo.city}, ${geo.country}`);
...
app.post('/ai', async (req, res) => {
    ...
    if (imageB64 && imageB64.length > MAX_IMAGE_B64) {
        pushLog(errorLog, { id, ts: new Date(), ip, type: 'INPUT', message: 'image too large' });
    ...
    pushLog(requestLog, { id, ts: new Date(), ip, promptSnippet: prompt.slice(0, 120),
        answer: answer.slice(0, 80), isVision: !!imageB64, status: 200, ms });
...
app.post('/admin/whitelist/:id/label', requireAdmin, (req, res) => {
    ...
    u.label = (req.body.label || '').trim().slice(0, 60);
    saveUsers(users);
    res.json({ ok: true, label: u.label });
});

app.post('/admin/clear', requireAdmin, (req, res) => {
    requestLog.length = 0; errorLog.length = 0;
...
app.listen(PORT, () => {
    ...
    console.log(`[examhub-proxy] whitelist: /whitelist/verify | /whitelist/request`);
